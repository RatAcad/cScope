...
load preprocess_data_3; clear mov; load Clocks_3; load params
%greenmov=greenmov;
%bluemov=bluemov;
cropme=0;
figure
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%REMOVE ANOMALIES MORE THAN 4 STANDARD DEV AWAY - RA 3/23/2020
% mB=squeeze(mean(mean(bluemov))); sB = mB>(mean(mB)+(std(mB)*4));
% blue_clock(sB)  = []; green_clock(sB)  = [];
% bluemov(:,:,sB) = []; greenmov(:,:,sB) = [];
% 
% mG=squeeze(mean(mean(greenmov))); sG = mG>(mean(mG)+(std(mG)*4));
% blue_clock(sG)  = []; green_clock(sG)  = [];
% bluemov(:,:,sG) = []; greenmov(:,:,sG) = [];
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


display('Computing');
% meanproj = nanmean(greenmov,3); 
% vascMask = single(removeVasculature_LP(meanproj)); vascMask(vascMask==1)=nan;
...
clock=blue_clock(1):TPF:blue_clock(end); % RA 12/9 - changed blue_clock(start)
figure;
...
Alpha_map=SF_map;
savefast Alpha_map2_3 Alpha_map  errors% bluemov greenmov blue_clock green_clock
